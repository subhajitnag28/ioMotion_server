...
export class UserController {
                    ...
                    user.find({
                        ...
                        if (err) {
                            ...
                            if (docs.length != 0) {
                                ...
                                user.save(data, function (err: any, success: any) {
                                    if (err) {
                                        res.status(500).json({
                                            success: false,
                                            data: err
                ...
                if (err) {
                    ...
                    user.find({
                        ...
                        if (docs.length != 0) {
                            ...
                            userTemp.find({
                                email: data.email
                            }).toArray(function (err1: any, response1: any) {
                                if (err1) {
                                    res.status(500).json({
                                        success: false,
                                        data: {
        ...
        if (request_body.email && request_body.password && request_body.otp) {
            ...
            Joi.validate(request_body, schema, function (err, value) {
                if (err) {
                    ...
                    useTemp.find({
                        email: data.email
                    }).toArray(function (err: any, res1: any) {
                        if (err) {
                            res.status(500).json({
                                success: false,
                                data: {
     ...
     */
    public getUsersByRole(req: Request, res: Response) {
        const request_body = req.body;
                            ...
                            if (err) {
                                ...
                                if (pageNo < 0 || pageNo === 0) {
                                    ...
                                    user.find({
                                        ...
                                        .toArray(function (err1: any, success1: any) {
                                            if (err1) {
                                                res.status(500).json({
                                                    success: false,
                                                    data: err1
     ...
     */
    public getUsersByRoleAndCustomerId(req: Request, res: Response) {
        const request_body = req.body;
     ...
     */
    public searchUserByCustomer(req: Request, res: Response) {
        const request_body = req.body;
    ...
    /**
        ...
        */
    public userAccessToInstallMicroservices(req: Request, res: Response) {
        const request_body = req.body;
        if (request_body._id) {
            const user = db.get().collection('user');
            user.find({
